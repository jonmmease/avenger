...
fn partition_expr_node(
    ...
    let Some(channel) = subplot
        .data_context_ref()
        .channels()
        .get(POLAR_SUBPLOT_PARTITION_CHANNEL)
    else {
        return Ok(None);
    };
    ...
    let Some(expr) = channel.expr(session_context) else {
        return Err(AvengerChartError::InvalidArgument(
            "Polar subplot partition_by does not support conditional values".to_string(),
        ));
    };
...
fn validate_partitioned_subplot(
    ...
    if subplot.has_plot_level_data() {
        return Err(AvengerChartError::InvalidArgument(
            "Partitioned Polar subplots inherit parent data; remove plot-level data from the child plot".to_string(),
        ));
    }

    for channel_name in ["r", "theta"] {
        let Some(channel) = subplot.data_context_ref().channels().get(channel_name) else {
            return Err(AvengerChartError::InvalidArgument(format!(
                "Partitioned Polar subplots require channel `{channel_name}`"
            )));
        };
        let Some(expr) = channel.expr(session_context) else {
            return Err(AvengerChartError::InvalidArgument(format!(
                "Partitioned Polar subplot channel `{channel_name}` does not support conditional values"
            )));
        };

        if !valid_partition_position_expr(&expr, partition_expr) {
            return Err(AvengerChartError::InvalidArgument(format!(
                "Partitioned Polar subplot channel `{channel_name}` must be an aggregate, literal/constant, or the partition expression"
            )));
        }
    }
...
impl SubplotContainerCoordinateSystem for Polar {
    async fn compile_subplot_mark(
        ...
        subplot.validate_no_facet_channels("Polar")?;
        let partition_expr = partition_expr_node(subplot, session_context)?;

        Ok(Arc::new(CompiledPolarSubplot {
...
/// Compiled child-plot mark positioned by Polar r/theta channels.
#[serde_as]
#[derive(Clone, Serialize, Deserialize)]
pub struct CompiledPolarSubplot {
...
impl CompiledPolarSubplot {
    ...
    pub fn group_name(&self, child_index: usize) -> String {
        match self.key() {
            ...
            None => format!("polar_subplot_{}_{}", self.mark_index(), child_index),
        }
    }
}

impl CompiledMarkCore for CompiledPolarSubplot {
    ...
    fn supported_channels(&self) -> Vec<ChannelDescriptor> {
        vec![
            ...
            ChannelDescriptor {
                name: POLAR_SUBPLOT_PARTITION_CHANNEL,
                required: false,
                default_value: None,
                allow_column_ref: true,
            },
...
#[typetag::serde]
#[async_trait::async_trait]
impl CompiledMark for CompiledPolarSubplot {
    async fn render_from_data(
        ...
        Err(AvengerChartError::InternalError(
            "Polar subplot marks require the top-level layout render dispatcher".to_string(),
        ))
    }
}
