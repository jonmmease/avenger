...
use avenger_chart_marks::{Line, Symbol, Text};
...
/// Polar position-channel builders for the generic `Text` mark.
pub trait PolarTextPositionChannels: Sized {
    fn r<V: Into<ChannelValue>>(self, value: V) -> Self;
    fn r_with<V, F>(self, value: V, f: F) -> Self
    where
        V: Into<ChannelValue>,
...
impl PolarTextPositionChannels for Text<Polar> {
    fn r<V: Into<ChannelValue>>(self, value: V) -> Self {
        self.with_channel_value("r", value.into())
    }
    ...
    where
        ...
        F: FnOnce(PolarPositionConfig) -> PolarPositionConfig,
    {
        configure_text_polar_position_channel(self, "r", value.into(), f)
    }
    ...
    where
        ...
        F: FnOnce(PolarPositionConfig) -> PolarPositionConfig,
    {
        configure_text_polar_position_channel(self, "theta", value.into(), f)
    }
...
fn configure_text_polar_position_channel(
    mark: Text<Polar>,
    channel_name: &str,
    channel_value: ChannelValue,
    f: impl FnOnce(PolarPositionConfig) -> PolarPositionConfig,
