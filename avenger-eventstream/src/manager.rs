...
mod tests {
    ...
    async fn application_wakeups_are_delivered_and_private_wakeups_stay_internal() {
        ...
        let status = manager
            .dispatch_event(
                &WindowEvent::WindowResize(WindowResizeEvent { size: [5.0, 6.0] }),
                &empty_rtree(),
                now,
            )
            .await;
        ...
        assert!(matches!(
            ...
            [SceneGraphEvent::WindowResize(_)]
        ));
    }
    ...
    async fn debounced_consuming_stream_stops_propagation_at_admission() {
        ...
        manager
            ...
            .await;
        assert_eq!(labels.lock().unwrap().as_slice(), &["first:WindowResize"]);
    }
    ...
    async fn rejected_handler_does_not_advance_previous_or_consume() {
        ...
        for (offset, size) in [[1.0, 1.0], [2.0, 2.0]].into_iter().enumerate() {
            manager
                .dispatch_event(
                    &WindowEvent::WindowResize(WindowResizeEvent { size }),
                    &empty_rtree(),
    ...
    async fn event_stream_debounce_commits_only_latest_event_on_matching_wake() {
        ...
        let first = manager
            .dispatch_event(
                &WindowEvent::WindowResize(WindowResizeEvent { size: [1.0, 1.0] }),
                &empty_rtree(),
                start,
            )
            .await;
        let second = manager
            .dispatch_event(
                &WindowEvent::WindowResize(WindowResizeEvent { size: [2.0, 2.0] }),
                &empty_rtree(),
