...
impl SyntaxNode {
    ...
    /// text.
    #[track_caller]
    pub fn warn_at(&mut self, Range { start, end }: Range<usize>, message: impl Into<EcoString>) {
    ...
    /// node.
    #[track_caller]
    pub fn hint_at(&mut self, Range { start, end }: Range<usize>, hint: impl Into<EcoString>) {
        assert!(end <= self.len()); // This isn't checked by `SubRange::new`.
        let sub_range = SubRange::new(start, end).expect("a valid sub-range");
        let hints = self.hints_mut().expect("expected an error or warning");
    ...
    pub fn diagnosis(&self) -> Diagnosis {
        let diagnosis = match self.node_ref() {
            ...
            NodeRef::Error(_) => Diagnosis {
                errors: true,
                warnings: false,
            },
        };
        match &self.data {
            Node::Warning(_, _) => Diagnosis {
                warnings: true,
    ...
    pub(crate) fn find_number(&self, target: SpanNumber) -> Option<Self> {
        ...
        if self.node.is_inner() && number < target.0 {
            ...
            while let Some(child) = children.next() {
                ...
                if children
                    .peek()
                    .is_none_or(|next| next.span().number() > target.0)
