        ...
        for c in path.components() {
            let comp = match c {
                ...
                path::Component::Normal(s) => {
                    ...
                    let segment =
                        Segment::new(string).map_err(|s| VirtualizeError::Invalid(s.into()))?;
...
impl<'a> Segment<'a> {
    ...
    fn realize(self) -> Result<&'a OsStr, RealizeError> {
        ...
        match (iter.next(), iter.next()) {
            ...
            (Some(other), _) => Err(RealizeError::Invalid(
                other.as_os_str().to_string_lossy().into(),
            )),
        }
    }
}
...
impl Segments {
    ...
    fn get_without_slash(&self) -> &str {
        self.0
            ...
            .expect("path to start with slash")
    }
...
mod tests {
    ...
    fn test_realize_windows() {
        ...
        assert_eq!(
            path("a/LPT\u{00b2} .baz/b").realize(root),
            Err(invalid("LPT² .baz"))
        );
    }
