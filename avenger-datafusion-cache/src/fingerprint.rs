...
///
/// Separates this key format from other uses of the same hashing scheme.
...
/// versions, the DataFusion version, and the execution
...
pub trait CacheVersionProvider: Send + Sync {
    ...
    /// recognize the node. The version identifies both the dataset and its
    /// revision: equal versions must describe equal partition contents.
...
pub enum ExclusionReason {
    ...
    /// bare index. Identical proto bytes can read different subquery results.
    SubqueryRef,
...
/// execution configuration, the registered
...
impl FingerprintContext {
    ...
    /// Hash of execution configuration included in fingerprints.
...
/// Hash all configuration entries, sorted by key for deterministic ordering.
...
pub(crate) fn node_fingerprint(
    ...
    if matches!(plan.name(), "DataSinkExec" | "AnalyzeExec") {
        return FingerprintOutcome::Excluded(ExclusionReason::UnsupportedNode {
            message: "execution has side effects or reports execution-specific metrics".into(),
        });
    }
    if plan
        .downcast_ref::<crate::runtime_parameters::RuntimeParameterExec>()
        .is_some()
    {
        return FingerprintOutcome::Excluded(ExclusionReason::UnsupportedNode {
            message: "runtime parameter values are execution-specific".to_string(),
        });
    }
    ...
    if let Some(exec) = plan.downcast_ref::<DataSourceExec>()
        && let Some(memory) = exec.data_source().downcast_ref::<MemorySourceConfig>()
    {
    ...
    if children.is_empty()
        && let Some(version) = ctx.provider_version(plan.as_ref())
    {
...
/// stable across separately constructed schemas.
///
...
fn node_local_bytes(plan: &Arc<dyn ExecutionPlan>) -> std::result::Result<Vec<u8>, NodeBytesError> {
...
/// Return whether a physical-plan node is structurally eligible to live in a
/// reusable prototype. This is deliberately stricter than DataFusion's reset
/// API: extension nodes must be encodable by the same guarded proto surface we
/// trust for cache fingerprints, and runtime-mutated or volatile expressions
/// are rejected. Scalar-subquery references are allowed here because their
/// shared results containers are cleared by `reset_plan_states`. The caller
/// must serialize executions and discard prototypes after cancellation.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum ReusablePrototypeNodeRejection {
    DynamicExpression,
    VolatileExpression,
    UnsupportedNode,
}

pub(crate) fn reusable_prototype_node_rejection(
    plan: &Arc<dyn ExecutionPlan>,
) -> Option<ReusablePrototypeNodeRejection> {
    if plan
        .downcast_ref::<crate::runtime_parameters::RuntimeParameterExec>()
        .is_some()
    {
    ...
    match node_local_bytes(plan) {
        Ok(_) | Err(NodeBytesError::Guard(GuardViolation::SubqueryRef)) => None,
        // These producers initialize a private dynamic-filter object even
        // when every dynamic-filter pushdown option is disabled. Prototype
        // planning forces those options off, so no consumer is wired to the
        // object and the dormant producer-local state cannot affect output.
        Err(NodeBytesError::Guard(GuardViolation::Dynamic))
            if matches!(plan.name(), "AggregateExec" | "HashJoinExec")
                || plan.name().starts_with("SortExec") =>
        {
            None
        }
        Err(NodeBytesError::Guard(GuardViolation::Dynamic)) => {
            Some(ReusablePrototypeNodeRejection::DynamicExpression)
        }
        Err(NodeBytesError::Guard(GuardViolation::Volatile)) => {
            Some(ReusablePrototypeNodeRejection::VolatileExpression)
        }
        Err(NodeBytesError::Unsupported(_)) => {
            Some(ReusablePrototypeNodeRejection::UnsupportedNode)
        }
    }
}
...
/// Inspect nested scalar-subquery references before proto serialization,
/// which records their results index but omits the captured results.
...
impl PhysicalProtoConverterExtension for GuardedProtoConverter {
    ...
    fn physical_expr_to_proto(
        ...
        // Scalar-subquery references own execution-local result containers and
        // consequently report a non-zero snapshot generation. Classify them
        // before the generic dynamic-expression guard so prototype planning
        // can explicitly allow this resettable case.
...
#[derive(Debug, Default)]
struct MemoInner {
    entries: HashMap<MemoryIdentity, MemoEntry>,
    insertions: u64,
    hits: u64,
}
                        ...
                        .map(|batch| {
                            let arrays = batch
                                ...
                                .map(|column| {
                                    ...
                                    Arc::as_ptr(column) as *const () as usize
                                })
                                .collect();
                            (batch.num_rows(), arrays)
                        })
                        .collect()
                })
                .collect(),
...
mod tests {
    ...
    #[test]
    fn content_memo_distinguishes_zero_column_row_counts() {
        let memo = ContentHashMemo::default();
        ...
        let first = memo.version_for(&[vec![batch(1)]], &schema).unwrap();
        let second = memo.version_for(&[vec![batch(9)]], &schema).unwrap();
        ...
        for ((name_a, out_a), (name_b, out_b)) in fps_a.iter().zip(fps_b.iter()) {
            ...
            assert_eq!(
                ...
                "node '{name_a}' fingerprint not deterministic across replans \
                 (schema carries multi-entry metadata)"
            );
            assert!(
    ...
    async fn empty_exec_placeholder_gives_child_independent_node_bytes() {
        ...
        let bytes_a = node_local_bytes(&filter_a).ok().unwrap();
        let bytes_b = node_local_bytes(&filter_b).ok().unwrap();
        assert_eq!(
            bytes_a, bytes_b,
    ...
    #[test]
    fn configuration_changes_invalidate_fingerprints() {
        ...
        let mut changed = ConfigOptions::default();
        changed.optimizer.filter_null_join_keys = !base.optimizer.filter_null_join_keys;
        assert_ne!(base_fp, config_fingerprint(&changed));
