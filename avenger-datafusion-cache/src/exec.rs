...
impl ExecutionPlan for CacheWriteExec {
    ...
    fn execute(
        ...
        let inner = match self.child.execute(partition, context) {
            Ok(inner) => inner,
            Err(error) => {
                self.handle.fail(partition);
                return Err(error);
            }
        };
        self.handle.start_partition(partition);
...
///   last one);
/// - dropped before the end → the entry is discarded immediately.
struct TeeStream {
...
mod tests {
    ...
    #[tokio::test]
    async fn overlapping_partition_executions_do_not_cache_duplicate_rows() {
        let cache = EvaluationCache::new(EvaluationCacheConfig::default());
        let child = memory_exec(&[vec![batch(&[1, 2, 3])]]);
        let handle = cache.begin_write(candidate_for(1, &child)).unwrap();
        let write = CacheWriteExec::new(child, handle);
        let mut first = write.execute(0, task_ctx()).unwrap();
        let mut second = write.execute(0, task_ctx()).unwrap();
        assert_eq!(first.next().await.unwrap().unwrap().num_rows(), 3);
        assert_eq!(second.next().await.unwrap().unwrap().num_rows(), 3);
