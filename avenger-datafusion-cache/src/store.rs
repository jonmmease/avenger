...
//!
//! [`EvaluationCache`] owns committed and pending entries
...
impl EvaluationCache {
    ...
    pub fn should_admit(&self, candidate: &CacheCandidate) -> AdmissionDecision {
        ...
        if let Some(elapsed) = elapsed
            && elapsed < self.config.min_elapsed_for_admission
        {
        ...
        if let Some(bytes) = estimated_bytes
            && bytes > self.config.max_entry_bytes
        {
    ...
    fn commit(&self, id: &Arc<()>, entry: CommittedEntry, observed_elapsed: Option<Duration>) {
        let key = entry.key;
        let fingerprint = key.subtree_fingerprint;
        let bytes = entry.actual_bytes;
        let mut inner = self.inner.lock().unwrap();
        let waiters = match inner.entries.get_mut(&key) {
            Some(EntryState::Pending(pending)) if Arc::ptr_eq(&pending.id, id) => {
        ...
        if let Some(elapsed) = observed_elapsed
            && let Some(record) = inner.observations.get_mut(&fingerprint)
        {
        ...
        while inner.bytes_total > self.config.max_memory_bytes
            || inner
                .entries
                .values()
                .filter(|entry| matches!(entry, EntryState::Committed(_)))
                .count()
    ...
    fn discard(&self, key: &PhysicalCacheKey, id: &Arc<()>) {
        let mut inner = self.inner.lock().unwrap();
        if matches!(inner.entries.get(key), Some(EntryState::Pending(pending)) if Arc::ptr_eq(&pending.id, id))
        {
...
impl CacheWriteHandle {
    ...
    /// Staging after partition completion discards the write.
    ...
    pub(crate) fn start_partition(&self, partition: usize) {
        let mut staging = self.inner.staging.lock().unwrap();
        if staging.finished {
            return;
        }
        if staging.partitions[partition].started {
            drop(staging);
            self.fail(partition);
        } else {
            staging.partitions[partition].started = true;
        }
    }
}
...
impl WriteHandleInner {
    ...
    fn stage(&self, partition: usize, batch: RecordBatch) {
        ...
        if staging.partitions[partition].complete
            || batch_bytes > self.max_entry_bytes - staging.bytes
        {
            staging.discard();
            drop(staging);
            if let Some(cache) = self.cache.upgrade() {
                cache.discard(&self.key, &self.id);
            }
            return;
        }
...
impl Drop for WriteHandleInner {
    fn drop(&mut self) {
        ...
        if !finished && let Some(cache) = self.cache.upgrade() {
            cache.discard(&self.key, &self.id);
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn empty_results_are_bounded_by_entry_count() {
        let cache = EvaluationCache::new(EvaluationCacheConfig {
            max_entries: 1,
            ..config()
        });
        for id in [1, 2] {
            cache
                .begin_write(candidate(id, 1))
                .unwrap()
                .complete_partition(0);
        }
    ...
    fn eviction_never_touches_pending() {
        let entry_bytes = batch(&[1, 2, 3]).get_array_memory_size();
        let cache = EvaluationCache::new(EvaluationCacheConfig {
            max_memory_bytes: entry_bytes, // holds one committed entry
            ..config()
        });
    ...
    #[test]
    fn cleared_write_cannot_replace_or_discard_a_new_write() {
        for complete_old in [false, true] {
            let cache = EvaluationCache::new(config());
            let old = cache.begin_write(candidate(1, 1)).unwrap();
            old.stage(0, batch(&[1]));
            cache.clear();
            let new = cache.begin_write(candidate(1, 1)).unwrap();
            if complete_old {
                old.complete_partition(0);
            }
            drop(old);
            assert!(matches!(cache.lookup(&key(1)), CacheLookup::Pending(_)));
            new.stage(0, batch(&[2, 3]));
            new.complete_partition(0);
            let CacheLookup::Ready(entry) = cache.lookup(&key(1)) else {
                panic!("new write must survive completion or cancellation of the old write");
            };
            assert_eq!(entry.rows(), 2);
            assert_eq!(cache.metrics().bytes, entry.bytes());
        }
    }
}
