...
#[tokio::test]
#[ignore = "benchmark: run explicitly with --ignored bench_shared"]
...
#[tokio::test]
async fn sliced_batches_do_not_share_results() {
...
#[tokio::test]
async fn repeated_writes_are_never_replaced_by_cached_counts() {
    let (ctx, cache) = cached_context(EvaluationCacheConfig {
        min_seen_count: 1,
        ..Default::default()
    });
    ctx.register_table(
        "output",
        Arc::new(MemTable::try_new(sales_schema(), vec![vec![]]).unwrap()),
    )
    .unwrap();
    for _ in 0..3 {
        run(&ctx, "INSERT INTO output SELECT * FROM sales").await;
    }
    let rows: usize = run(&ctx, "SELECT * FROM output")
        .await
        .iter()
        .map(RecordBatch::num_rows)
        .sum();
    assert_eq!(rows, 3 * sales_batch().num_rows());
    assert!(
        cache.metrics().hits > 0,
        "the read-only input can still be reused"
    );
}
