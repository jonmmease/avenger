...
/// Generate test data for each Vega spec located in `gen-test-data/vega-specs`
/// For each spec, the following three files are saved to `vega-wgpu-renderer/tests/specs`
    ...
    visit_dirs(specs_path, &mut move |dir_entry| {
        ...
        if let Some(spec_name) = spec_file_name.strip_suffix(".vg.json") {
            ...
            let output_dir = format!(
                "{}/../vega-wgpu-renderer/tests/specs/{spec_category}/",
                env!("CARGO_MANIFEST_DIR")
            );
