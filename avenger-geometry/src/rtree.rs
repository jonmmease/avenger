...
use geo::{BoundingRect, Centroid, CoordsIter, Distance, Euclidean, Intersects, Relate};
...
pub enum GeometryQueryHitPolicy {
    /// The geometry envelope, expanded by half the stroke width, intersects the region.
    EnvelopeIntersects,
    /// The geometry or its stroke intersects the region, using the point-picking distance model.
    GeometryIntersects,
    /// The complete geometry and its stroke lie within the region, including its boundary.
    GeometryContained,
    /// The explicit placement point lies in the region. Marks without an anchor do not match.
    AnchorInside,
    /// The geometric centroid lies in the region. Empty geometries do not match.
    CentroidInside,
}
...
pub struct GeometryInstance {
    ...
    /// Scene-space placement point. Text includes its label offsets. Whole line,
    /// area, and trail marks have no single placement point and use `None`.
...
impl SceneGraphRTree {
    fn new(
        ...
        let next_render_order = geometries.len();
    ...
    /// policy is applied to the candidate geometries. Anchor queries inspect all
    /// instances because an anchor can lie outside the geometry envelope. Results
    /// are sorted by scene path and instance index for deterministic event handling.
    pub fn query_shape(
        &self,
        ...
        // A mark anchor can lie outside its painted geometry, such as an arc center.
        let candidates: Box<dyn Iterator<Item = &GeometryInstance> + '_> = match hit_policy {
            GeometryQueryHitPolicy::AnchorInside => Box::new(self.rtree.iter()),
            _ => Box::new(self.rtree.locate_in_envelope_intersecting(&envelope)),
        };
    ...
    pub fn insert(&mut self, geometry: GeometryInstance) {
        ...
        self.render_orders
            ...
            .or_default()
            .insert(
                geometry.mark_instance.instance_index,
                self.next_render_order,
            );
...
fn query_shape_hits(
    ...
    match hit_policy {
        ...
        GeometryQueryHitPolicy::AnchorInside => instance
            .anchor
            .is_some_and(|anchor| query_shape_contains_point(shape, anchor)),
        GeometryQueryHitPolicy::CentroidInside => instance
            .geometry
            .centroid()
            .is_some_and(|point| query_shape_contains_point(shape, [point.x(), point.y()])),
...
fn query_shape_contains_point(shape: &GeometryQueryShape, point: [f32; 2]) -> bool {
    match shape {
        ...
        GeometryQueryShape::Polygon { points } => {
            ...
            polygon.intersects(&geo_types::Point::new(point[0], point[1]))
        }
    }
}
...
fn query_shape_intersects_geometry(
    ...
    match shape {
        GeometryQueryShape::Rect { .. } | GeometryQueryShape::Polygon { .. } => {
            ...
            Euclidean::distance(&query_geometry, &instance.geometry) <= instance.half_stroke_width
        }
        GeometryQueryShape::Circle { cx, cy, radius } => {
...
fn query_shape_contains_geometry(shape: &GeometryQueryShape, instance: &GeometryInstance) -> bool {
    match shape {
        GeometryQueryShape::Circle { cx, cy, radius } => {
            let remaining_radius = radius.abs() - instance.half_stroke_width;
            remaining_radius >= 0.0
                && instance.geometry.coords_count() > 0
                && instance
                    .geometry
                    .coords_iter()
                    .all(|point| (point.x - cx).hypot(point.y - cy) <= remaining_radius)
        }
        GeometryQueryShape::Rect { x0, y0, x1, y1 } => {
            let envelope = instance.envelope();
            envelope.lower()[0] >= x0.min(*x1)
                && envelope.lower()[1] >= y0.min(*y1)
                && envelope.upper()[0] <= x0.max(*x1)
                && envelope.upper()[1] <= y0.max(*y1)
        }
        GeometryQueryShape::Polygon { points } => {
            let Some(polygon) = polygon_from_points(points) else {
                return false;
            };
            polygon.relate(&instance.geometry).is_covers()
                && (instance.half_stroke_width == 0.0
                    || Euclidean::distance(polygon.exterior(), &instance.geometry)
                        >= instance.half_stroke_width)
        }
    }
}
