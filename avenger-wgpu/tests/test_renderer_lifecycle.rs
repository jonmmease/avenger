...
fn renderer_resize_preserves_logical_mark_coordinates() {
    ...
    reused.resize(large);
    let mut target = OffscreenTarget::new(&device, &OffscreenTargetDescriptor::new(large, format));
    reused
...
fn configured_text_engine_controls_the_rendered_font_and_rich_limit() {
    ...
    let render = |mark: SceneTextMark, config: CanvasConfig| {
        ...
        let mut target =
            OffscreenTarget::new(&device, &OffscreenTargetDescriptor::new(dimensions, format));
...
#[test]
fn instancing_preserves_translucent_stroke_compositing() {
    ...
    let mut render = |len| {
        let mark = SceneSymbolMark {
            len,
            x: (0..len)
                .map(|i| if i == 0 { 75.0 } else { -1000.0 })
                .collect::<Vec<_>>()
                .into(),
            y: 75.0.into(),
            size: 3600.0.into(),
            fill: ColorOrGradient::Color([1.0, 0.0, 0.0, 1.0]).into(),
            stroke: ColorOrGradient::Color([0.0, 0.0, 1.0, 0.5]).into(),
            stroke_width: Some(20.0),
            ..Default::default()
        };
        canvas
            .set_scene(&SceneGraph {
                width: 150.0,
                height: 150.0,
                origin: [0.0, 0.0],
                marks: vec![mark.into()],
    ...
    let normal = render(1);
    let instanced = render(100);
    assert_eq!(
        normal.get_pixel(100, 75),
