...
fn create_font_string_from_parts(
    font: &str,
    font_size: f32,
    ...
    format!("{style} {weight} {}px {}", font_size * scale, font)
}
...
fn measure_font_metrics_with_canvas(
    config: &FontMetricsConfig,
    font_str: &str,
    scale: f32,
) -> Result<FontMetrics, wasm_bindgen::JsValue> {
    let offscreen_canvas = OffscreenCanvas::new(400, 400)?;
    let context = offscreen_canvas.get_context("2d")?.unwrap();
    let text_context = context.dyn_into::<OffscreenCanvasRenderingContext2d>()?;

    text_context.set_font(font_str);
    let metrics = text_context.measure_text("")?;

    let ascent = metrics.font_bounding_box_ascent() as f32 / scale;
    let descent = metrics.font_bounding_box_descent() as f32 / scale;
    let height = ascent + descent;
    let line_height = height.max(config.font_size);

    Ok(FontMetrics {
        ascent,
        descent,
        height,
        line_gap: (line_height - height).max(0.0),
        line_height,
    })
}
...
impl TextMeasurer for HtmlCanvasTextMeasurer {
    ...
    fn measure_font_metrics(&self, config: &FontMetricsConfig) -> FontMetrics {
        ...
        // in base coordinates.
        let scale = 1.0;
        let font_str = create_font_string_from_parts(
            config.font,
            config.font_size,
            config.font_weight,
            config.font_style,
        ...
        measure_font_metrics_with_canvas(config, &font_str, scale)
            .unwrap_or_else(|_e| FontMetrics::fallback(config.font_size))
    }
}
