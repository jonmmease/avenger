...
pub struct PatternGeometry {
    ...
    pub ink: [f32; 4],
    /// Compound coverage path. Renderers should fill or mask this once.
...
mod tests {
    ...
    fn horizontal_stripes_generate_one_quad_per_centerline() {
        ...
        let geometry = build_pattern_geometry(
            &fill,
        ...
        assert_eq!(begin_count(&geometry.coverage_path), 3);
        assert_eq!(begins(&geometry.coverage_path)[0], [-1.0, -1.0]);
    }
    ...
    fn stripe_phase_offsets_centerlines_along_normal() {
        ...
        assert_eq!(begin_count(&geometry.coverage_path), 2);
        assert_eq!(begins(&geometry.coverage_path)[0], [-1.0, 7.0]);
    }
    ...
    fn dash_phase_uses_global_stripe_coordinate() {
        ...
        assert_eq!(begins(&geometry.coverage_path)[0], [2.0, -1.0]);
    }
    ...
    fn plot_and_mark_anchors_produce_different_origins() {
        ...
        let plot_geometry = build_pattern_geometry(&plot_pattern, &ctx)
            .unwrap()
            .unwrap();
        let mark_geometry = build_pattern_geometry(&mark_pattern, &ctx)
            .unwrap()
            .unwrap();

        assert_eq!(begins(&plot_geometry.coverage_path)[0][1], 35.0);
        assert_eq!(begins(&mark_geometry.coverage_path)[0][1], 24.0);
    }
    ...
    fn plot_anchor_requires_plot_bounds() {
        ...
        assert_eq!(
            build_pattern_geometry(&pattern, &ctx).unwrap_err(),
    ...
    fn symbol_layers_generate_lattice_coverage() {
        ...
        let geometry = build_pattern_geometry(&pattern, &ctx).unwrap().unwrap();

        assert!(begin_count(&geometry.coverage_path) > 4);
        assert_ne!(begins(&geometry.coverage_path)[0], [-1.0, -1.0]);
    }
    ...
    fn open_symbol_layers_generate_stroked_coverage() {
        ...
        assert!(begin_count(&geometry.coverage_path) > 8);
    }
