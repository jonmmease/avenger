...
use datafusion::arrow::array::{StringArray, Float64Array, Int32Array, RecordBatch};
...
async fn main() -> datafusion::error::Result<()> {
    ...
    let batch = RecordBatch::try_new(
        ...
        vec![
            Arc::new(StringArray::from(vec![
                ...
                "Mouse", "Keyboard", "Monitor", "Tablet", "Phone"
            ])),
            Arc::new(StringArray::from(vec![
            ...
            Arc::new(Float64Array::from(vec![
                1200.0, 25.0, 75.0, 350.0, 1500.0,
                30.0, 80.0, 400.0, 800.0, 900.0
            ])),
    ...
    let _binned_rect = Rect::<Cartesian>::new()
        .data(df.clone())
        .transform(
            Bin::x("weight")
    ...
    let _price_histogram = Rect::<Cartesian>::new()
        .data(df.clone())
        .transform(
            Bin::x("price")
                .bins(5)
                .aggregate(count(lit(1)))
                .extra_aggregate("fill", datafusion::functions_aggregate::expr_fn::avg(col("quantity")))
        )?;
        // Note: The transform would automatically set:
    ...
    for channel in output_channels {
        println!("  - {}: {} ({})", channel.name, channel.description, channel.data_type);
    }
    ...
    let category_agg = df.clone()
        .aggregate(
            vec![col("category")],
            vec![sum(col("quantity"))]
        )?;
    ...
    let _histogram_plot = Plot::new(Cartesian)
        .width(800.0)
        .height(600.0)
        .mark(
            Rect::new()
    ...
    let _scatter_plot = Symbol::<Cartesian>::new()
        ...
        .transform(
            Group::xy("category", "product", "size")
                .aggregate(sum(col("quantity")))
                .extra_aggregate("fill", datafusion::functions_aggregate::expr_fn::avg(col("price")))
        )?
