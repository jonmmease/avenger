...
impl CompiledPlot {
    ...
    /// Perform measurement pass and return a MeasurementResult for subsequent render.
    ///
    /// This method performs the complete measurement phase:
    /// 1. Computes layout to determine plot area dimensions
    /// 2. Builds initial scales with those dimensions
    /// 3. Evaluates marks to capture layout updates (scale domain changes, facet overflow)
    /// 4. Measures guide overflow
    /// 5. Computes final layout with overflow
    /// 6. **CRITICALLY**: Rebuilds scales with final layout dimensions
    /// 7. Packages everything into MeasurementResult
    ///
    /// The returned `MeasurementResult` contains scales with ranges that match the final
    /// plot area dimensions. The render pass can use these directly without recomputation.
    ///
    /// # Arguments
    ///
    ...
    pub async fn measure_for_render(
        &self,
        width: f32,
        height: f32,
        ctx: &SessionContext,
        ...
        // Inject canvas dimensions into params for media queries
        ...
        let (initial_plot_width, initial_plot_height, initial_layout) = if dimensions_are_plot_area
        {
            ...
            let plot_bounds = layout.plot_area_bounds();
            (plot_bounds.width, plot_bounds.height, layout)
        };

        // 3. Build scales with initial plot area dimensions
        let initial_scales = scale_provider
            .build_scales(initial_plot_width, initial_plot_height, ctx, &merged_params)
            .await?;
        ...
        // Add legend dimensions to overflow if in plot area mode
        if dimensions_are_plot_area {
            ...
            for (position, legend_keys) in &initial_layout.taffy_layout.legends_by_position {
                let max_dimension = legend_keys
                    .iter()
                    .filter_map(|key| initial_layout.taffy_layout.legends.get(key))
        ...
        // 6. Compute final layout with overflow (only for canvas mode)
        let (final_plot_width, final_plot_height, canvas_size, final_layout) =
            if !dimensions_are_plot_area {
                // Canvas mode: rebuild layout with measured overflow
                use crate::layout::ChartLayout;
                let theme = self.get_theme();
                let evaluated_spec = evaluate_layout_spec(
                ...
                let legend_measurements = self
                    .prepare_legend_measurements(
                        ...
                        &merged_scales,
                        taffy::Size {
                            width: initial_plot_width,
                            height: initial_plot_height,
                        },
                        ctx,
                        &merged_params,
                    )
                ...
                let final_layout = layout.compute(&evaluated_spec)?;
                let final_bounds = final_layout.plot_area_bounds();

                (
                    final_bounds.width,
                    final_bounds.height,
                    final_layout.canvas_size,
                    final_layout,
                )
            } else {
                // Plot area mode: dimensions are fixed, use initial layout
                (
                    initial_plot_width,
                    initial_plot_height,
                    initial_layout.canvas_size,
                    initial_layout,
                )
            };

        // 7. CRITICAL: Rebuild scales with final dimensions
        // This ensures scales have ranges matching the actual plot area
        ...
        let clip = if let Some(ref guide) = self.compiled_guide {
            let configured_scales: HashMap<String, ConfiguredScale> = final_merged_scales
                .iter()
                .map(|(k, v)| (k.clone(), v.configured().clone()))
                .collect();
            guide.get_clip(final_plot_width, final_plot_height, &configured_scales)
        } else {
        ...
        // 9. Create plot bounds
        let plot_bounds = crate::layout::LayoutBounds {
            x: final_layout.taffy_layout.plot_area.x,
            y: final_layout.taffy_layout.plot_area.y,
            width: final_plot_width,
            height: final_plot_height,
        };

        // 10. Package into MeasurementResult
        Ok(super::PlotMeasurementResult::new(
            (final_plot_width, final_plot_height),
            canvas_size,
            plot_bounds,
            clip,
            final_merged_scales,
    ...
    /// Render using pre-computed MeasurementResult.
    ///
    /// This method uses the scales and layout from MeasurementResult directly,
    /// WITHOUT recomputing layout or rebuilding scales. This ensures that marks
    /// are evaluated with the same dimensions that were used for measurement.
    ///
    /// # Arguments
    ///
    /// * `ctx` - DataFusion session context
    /// * `params` - Parameters for evaluation
    /// * `measurement` - Pre-computed measurement result from `measure_for_render()`
    /// * `data_override` - Optional data override for faceted subplots
    /// * `dimensions_are_plot_area` - If true, input was plot area mode; if false, canvas mode
    pub async fn render_with_measurement(
        &self,
        ctx: &SessionContext,
        params: &IndexMap<String, datafusion::common::ScalarValue>,
        measurement: &super::PlotMeasurementResult,
        data_override: Option<&DataFrame>,
        dimensions_are_plot_area: bool,
        ...
        merged_params.insert(
            "width".to_string(),
            datafusion::common::ScalarValue::Float32(Some(measurement.canvas_size.0)),
        );
        merged_params.insert(
            "height".to_string(),
            datafusion::common::ScalarValue::Float32(Some(measurement.canvas_size.1)),
        );

        // 2. Use pre-computed dimensions and scales from MeasurementResult
        let (plot_area_width, plot_area_height) = measurement.plot_area_size;

        if std::env::var("AVENGER_CHART_DEBUG_LAYOUT").is_ok() {
            eprintln!(
                "render_with_measurement: plot_area={}x{} (from MeasurementResult)",
        ...
        let plot_bounds_struct = measurement.plot_bounds;
        ...
        // 5. Create legends using pre-computed layout
        let legend_marks = self
            .create_legends_with_layout(
                &measurement.scales,
                &measurement.layout_result,
                ctx,
                &merged_params,
            )
            .await?;

        // 6. Create title and subtitle
        let title_marks = if let Some(title_bounds) = &measurement.layout_result.title {
        ...
        let subtitle_marks = if let Some(subtitle_bounds) = &measurement.layout_result.subtitle {
        ...
        // 7. Create debug marks if enabled
        let mut debug_marks = Vec::new();
        if std::env::var("AVENGER_CHART_DEBUG_LAYOUT").is_ok() {
            eprintln!(
                "render_with_measurement: layout plot_area width={}",
                measurement.layout_result.plot_area.width
            );
        ...
        // 8. Return PlotComponents
        Ok(super::PlotComponents {
            data_marks,
            guide_marks,
            legend_marks,
            title_marks,
            ...
            overflow: Some(measurement.overflow.clone()),
            debug_marks,
        })
    }
