...
impl ConfiguredScaleLegendExt for ConfiguredScale {
    fn domain_values(&self) -> Result<DomainValues, AvengerChartError> {
        ...
        match self.scale_impl.scale_type() {
            ...
            "threshold" => {
                // For threshold scales, we need one value per interval (n+1 intervals for n thresholds)
                // These values represent the intervals, not the thresholds themselves
                let threshold_count = domain_array.len();
                let interval_count = threshold_count + 1;

                // Create placeholder values for each interval
                // The actual labels are generated by domain_labels()
                let interval_values: Vec<ScalarValue> = (0..interval_count)
                    .map(|i| ScalarValue::Int32(Some(i as i32)))
                    .collect();
