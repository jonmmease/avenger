...
macro_rules! impl_mark_common {
    ($mark_type:ident, $mark_name:literal) => {
        impl<C: CoordinateSystem> $mark_type<C> {
            ...
            pub fn transform(mut self, transform: impl crate::transforms::Transform) -> Result<Self, crate::error::AvengerChartError> {
                // Apply transform to existing data context
                let ctx = std::mem::replace(&mut self.config.data, crate::transforms::DataContext::default());
