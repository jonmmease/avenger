...
use datafusion::functions::math::expr_fn::{sin, cos};
use datafusion::logical_expr::{lit, Expr};
use crate::scales::ScaleRange;
use crate::error::AvengerChartError;
use crate::axis::{CartesianAxis, AxisPosition, AxisTrait};
...
pub trait CoordinateSystem: Sized + Send + Sync + 'static {
    ...
    fn transform_expressions(&self, channels: HashMap<String, Expr>) -> Result<TransformResult, AvengerChartError>;
...
impl CoordinateSystem for Cartesian {
    ...
    fn transform_expressions(&self, mut channels: HashMap<String, Expr>) -> Result<TransformResult, AvengerChartError> {
        // Cartesian is identity transform - just pass through x and y
        let x = channels.remove("x")
            .ok_or_else(|| AvengerChartError::MissingChannelError("x".to_string()))?;
            
        let y = channels.remove("y")
    ...
    fn default_axis(channel: &str) -> Option<Self::Axis> {
        match channel {
            "x" => Some(CartesianAxis::new()
                .position(AxisPosition::Bottom)
                .label_angle(0.0)),
            "y" => Some(CartesianAxis::new()
...
impl CoordinateSystem for Polar {
    ...
    fn default_range(&self, channel: &str, width: f64, height: f64) -> Option<ScaleRange> {
        match channel {
            "theta" => Some(ScaleRange::new_interval(lit(0.0), lit(2.0 * std::f64::consts::PI))),
            "r" => {
