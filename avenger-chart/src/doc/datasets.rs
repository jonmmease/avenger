...
pub fn scatter_quadrants(ctx: &SessionContext) -> DataFrame {
    let batch = RecordBatch::try_from_iter(vec![
        (
            "x",
            Arc::new(Float64Array::from(vec![-3.0, -1.0, 0.5, 1.2, 2.5, 3.0]))
                as datafusion::arrow::array::ArrayRef,
        ),
        (
            "y",
            Arc::new(Float64Array::from(vec![2.0, -2.5, 1.5, -1.0, 2.8, -3.5]))
                as datafusion::arrow::array::ArrayRef,
        ),
    ])
...
pub fn categorical_bars(ctx: &SessionContext) -> DataFrame {
    let batch = RecordBatch::try_from_iter(vec![
        ...
        (
            "value",
            Arc::new(Float64Array::from(vec![28.0, 55.0, 43.0, 91.0, 81.0]))
                as datafusion::arrow::array::ArrayRef,
        ),
    ])
