                ...
                if let Some(expr) = facet_expr {
                    ...
                    for (col_idx, (domain_val, has_data)) in iteration_domain.iter().enumerate() {
                        ...
                        let subplot_measure_params = {
                            ...
                            // Col's orthogonal channel is "y". Track "y" for global edge if:
                            // 1. Parent is already tracking "y" (from unified_y_title), OR
                            // 2. "y" has Level(2+) sharing (multi-level sharing in same-type nesting)
                            let y_has_multilevel_sharing = merged_scale_sharing
                                .get("y")
                            ...
                            let (global_edge_tracked_channels, global_edge_channels) = if let Some(parent_ctx) = FacetContext::from_params(params) {
                                // Check if parent is tracking "y" OR if "y" has Level(2+) sharing
                                let parent_tracking_y = parent_ctx.global_edge_tracked_channels.contains("y");
                                let should_track_y = parent_tracking_y || y_has_multilevel_sharing;

                                if should_track_y {
                                    let (row, col) = (parent_row, col_idx);

                                    // Start with parent's tracked channels
                                    let mut tracked = parent_ctx.global_edge_tracked_channels.clone();
                                    if y_has_multilevel_sharing {
                                        tracked.insert("y".to_string());
                                    }

                                    // Inherit parent's global_edge_channels, or start fresh if parent wasn't tracking
                                    let parent_edge = if parent_tracking_y {
                                    ...
                                    let at_edge = parent_edge
                                        ...
                                        .filter(|ch| match ch.as_str() {
                                            "x" => row == parent_num_rows - 1, // Bottom edge
                                            "y" => col == 0,                   // Left edge
                                            _ => true,
                                        })
                                        .collect();
                            ...
                            } else if y_has_multilevel_sharing {
                                // No parent but "y" has Level(2+) - start tracking at top level
                                let mut tracked = std::collections::HashSet::new();
                                tracked.insert("y".to_string());
                                // Only mark as at-edge if at left column
                                let at_edge: std::collections::HashSet<String> = if col_idx == 0 {
                                    tracked.clone()
                                } else {
...
impl CompiledGuide for FacetColGuide {
    ...
    async fn evaluate(
        ...
        //
        // The cached _col_overflow may contain INFLATED values if our subplot contains
        // a nested FacetCol. This is because:
        // - FacetCol adds labels on TOP/BOTTOM
        // - FacetColGuide uses TOP/BOTTOM overflow to position its labels
        // - If subplot has nested FacetCol, its TOP/BOTTOM overflow includes those labels
        //
        // We need to re-compute intrinsic overflow (ignoring nested facet labels) when:
        // - The subplot contains a nested FacetCol (same-type nesting causes dimension overlap)
        //
        // For alternating nesting (FacetCol > FacetRow), the cached values are correct
        // because FacetRow adds labels on LEFT/RIGHT, which don't affect TOP/BOTTOM.
        use crate::facet::guide::shared::marks_contain_nested_facet_type;
        let has_same_type_nested_facet = self
            .facet_sources
            .iter()
            .any(|source| marks_contain_nested_facet_type(&source.subplot.marks, "facet_col"));
        ...
        let overflow_for_computation = if has_same_type_nested_facet {
            ...
            _col_overflow // Different-type or no nesting: cached values are correct
        };
