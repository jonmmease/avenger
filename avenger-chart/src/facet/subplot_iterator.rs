//! Iterator abstraction for consistent subplot iteration in faceted layouts (STUBBED)
//!
//! This module is stubbed as part of the facet fresh start refactoring.
//! The coordination context-dependent logic has been removed.
...
///
/// This version is STUBBED - coordination context support has been removed.
...
impl<DimConfig: FacetDimensionConfig> SubplotIterator<DimConfig> {
    ...
    ///
    /// Note: coordination_context parameter is ignored (stubbed)
    pub fn new(
        domain_vals: Vec<ScalarValue>,
        base_params: IndexMap<String, ScalarValue>,
        scale_sharing: std::collections::HashMap<String, ScaleSharing>,
        _coordination_context: Option<()>, // Stubbed - was FacetCoordinationContext
    ) -> Self {
        Self {
            domain_vals,
            base_params,
...
impl<DimConfig: FacetDimensionConfig> Iterator for SubplotIterator<DimConfig> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        // Get unified_channels from DimConfig
        let unified_channels = DimConfig::unified_channels()
            .iter()
            .map(|s| s.to_string())
            .collect();

        // Use default position/grid dimensions from DimConfig
        let position = DimConfig::index_to_position(index);
        let grid_dimensions = DimConfig::count_to_grid_dimensions(self.domain_vals.len());

        // Use scale_sharing as provided
        let scale_sharing = self.scale_sharing.clone();
