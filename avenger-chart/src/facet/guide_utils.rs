...
/// Configuration for rendering a unified axis title
#[derive(Clone, Debug)]
pub struct UnifiedTitleRenderConfig {
    /// The title text
    pub title: String,
    ...
    /// Font size in pixels
    pub font_size_px: f32,
    /// X position of the title center
    pub x: f32,
    /// Y position of the title center
    pub y: f32,
    /// Rotation angle in degrees (0 for horizontal, ±90 for vertical)
    pub angle: f32,
    /// Whether the axis is at the "far" edge (right for y-axis, bottom for x-axis)
    pub axis_at_far_edge: bool,
}

/// Render a unified axis title (x-axis or y-axis)
///
/// This creates a text mark for a unified axis title that spans across all facet cells.
/// The title is centered and optionally rotated for vertical axes.
pub fn render_unified_axis_title(
    config: &UnifiedTitleRenderConfig,
    theme: &Theme,
    theme_params: &IndexMap<String, ScalarValue>,
) -> SceneTextMark {
    let title_ctx = ThemeContext::new("guide", theme_params.clone())
    ...
    // Determine baseline based on rotation and position
    let baseline = if config.angle.abs() > 45.0 {
        // Vertical text (rotated)
        TextBaseline::Bottom
    } else if config.axis_at_far_edge {
        // Horizontal, at bottom/right edge
        TextBaseline::Top
    } else {
        // Horizontal, at top/left edge
        TextBaseline::Bottom
    };

    SceneTextMark {
        text: config.title.clone().into(),
        x: config.x.into(),
        y: config.y.into(),
        align: TextAlign::Center.into(),
        baseline: baseline.into(),
