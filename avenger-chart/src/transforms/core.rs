...
impl DataContext {
    ...
    pub fn with_encoding_expr(mut self, channel: &str, expr: datafusion::logical_expr::Expr) -> Self {
        // Resolve any channel references in the expression
        let resolved_expr = self.resolve_channel_refs(expr);
        
        // Store the encoding
        self.encodings.insert(channel.to_string(), ChannelValue::new(resolved_expr));
    ...
    fn resolve_channel_refs(&self, expr: datafusion::logical_expr::Expr) -> datafusion::logical_expr::Expr {
