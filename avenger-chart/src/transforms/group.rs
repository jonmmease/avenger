...
impl Group<Cartesian> {
    ...
    pub fn xy(x_field: impl Into<String>, y_field: impl Into<String>, agg_channel: &str) -> Self {
        Self::new(
            vec![(x_field.into(), "x"), (y_field.into(), "y")],
            agg_channel,
        )
    }
    ...
    pub fn xfill(x_field: impl Into<String>, fill_field: impl Into<String>) -> Self {
        Self::new(
            vec![(x_field.into(), "x"), (fill_field.into(), "fill")],
            "y",
        )
    }
    ...
    pub fn yfill(y_field: impl Into<String>, fill_field: impl Into<String>) -> Self {
        Self::new(
            vec![(y_field.into(), "y"), (fill_field.into(), "fill")],
            "x",
        )
    }
...
impl Group<Polar> {
    ...
    pub fn rtheta(
        r_field: impl Into<String>,
        theta_field: impl Into<String>,
        agg_channel: &str,
    ) -> Self {
        Self::new(
            vec![(r_field.into(), "r"), (theta_field.into(), "theta")],
            agg_channel,
        )
    }
}
...
impl<C: CoordinateSystem> Transform for Group<C> {
    fn transform(&self, ctx: DataContext) -> Result<DataContext, AvengerChartError> {
        ...
        for (field, channel) in &self.group_fields {
            result = result.with_encoding(channel, field).with_channel_metadata(
                channel,
                serde_json::json!({
                    "field": field,
        ...
        if let Some(ref expr) = self.primary_agg {
            result = result
                .with_encoding(
                    &self.default_agg_channel,
                    &format!("agg_{}", self.default_agg_channel),
                )
