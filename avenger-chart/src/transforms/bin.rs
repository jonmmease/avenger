    ...
    fn transform(&self, ctx: DataContext) -> Result<DataContext, AvengerChartError> {
        ...
        for (i, config) in self.configs.iter().enumerate() {
            if config.width.is_none() && config.bins.is_none() {
                return Err(AvengerChartError::InvalidArgument(
                    format!("Bin transform for field {} (index {}) requires either width or bins to be specified", 
                            config.field, i)
                ));
            }
        }
        ...
        for config in &self.configs {
            ...
            result = result
                ...
                .with_channel_metadata(&config.channel_start, serde_json::json!({
                    "transform": "bin",
                    "field": config.field,
                    "width": config.width,
                    "bins": config.bins,
        ...
        if let Some(ref _agg) = self.agg {
            ...
            result = result
                .with_channel_metadata("value", serde_json::json!({
