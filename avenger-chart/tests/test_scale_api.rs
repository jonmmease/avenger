...
fn test_typed_scale_with_band() {
    ...
    let _plot = Plot::<Cartesian>::new()
        ...
        .mark(Rect::new().x(col("category")).y(col("value")));
}
...
fn test_scale_preserves_domain_from_data() {
    ...
    let _plot = Plot::<Cartesian>::new()
        ...
        .mark(
            Rect::new().x(col("category")).y(col("value")), // This should automatically set up domain for y scale
        );
}
