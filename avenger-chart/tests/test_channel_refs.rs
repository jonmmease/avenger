...
fn test_channel_reference_basic() {
    ...
    let mark = Line::<Cartesian>::new()
        .data(df)
        .x("month")
        .y(":x");  // This should reference the :x channel
    ...
    assert_eq!(data_ctx.encoding_expr_string("x"), Some("month".to_string()));
    assert_eq!(data_ctx.encoding_expr_string("y"), Some("month".to_string())); // :x resolved to month
}
...
fn test_channel_reference_with_expression() {
    ...
    assert!(data_ctx.encoding_expr_string("stroke").unwrap().contains("sum")); // :y resolved to sum(sales)
}