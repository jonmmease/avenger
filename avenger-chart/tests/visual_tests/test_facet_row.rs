...
async fn facet_row_iris_scatter() {
    ...
    // Build inner subplot (Cartesian scatter)
    let inner = Plot::<Cartesian>::new().mark(
        Symbol::new()
            .x(col("sepal_length"))
            .y(col("sepal_width"))
            .size(36.0)
    ...
    let compiled_inner = inner.compile(&ctx).await.expect("compile inner");
    ...
    let outer = Plot::<FacetRow>::new()
        ...
        .mark(
            Facet::new()
                .row(col("species"))
                .subplot_compiled(&compiled_inner),
        );
