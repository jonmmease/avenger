...
fn vertical_colorbar_overlay() -> ColorbarOverlay {
    ColorbarOverlay::new().mark(
        Rect::<Cartesian>::new()
            ...
            .y_with(col("temperature_min"), |c| c.exclude_from_scale_domain())
            .y2_with(col("temperature_max"), |c| c.exclude_from_scale_domain())
...
fn horizontal_colorbar_overlay() -> ColorbarOverlay {
    ColorbarOverlay::new().mark(
        Rect::<Cartesian>::new()
            .data_store(StoreData::new("temperature_interval"))
            .x_with(col("temperature_min"), |c| c.exclude_from_scale_domain())
            .x2_with(col("temperature_max"), |c| c.exclude_from_scale_domain())
