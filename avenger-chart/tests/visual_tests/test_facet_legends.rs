...
/// Test 5: Grid facet with legends in each subplot
/// Tests 2D grid faceting with ordinal color legends
#[tokio::test]
async fn test_facet_grid_with_legends() {
    let ctx = SessionContext::new();
    let iris_path = format!("{}/tests/data/iris.parquet", env!("CARGO_MANIFEST_DIR"));
    let df = ctx
        .read_parquet(iris_path, ParquetReadOptions::default())
