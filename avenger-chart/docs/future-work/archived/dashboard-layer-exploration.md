# A Dashboard Layer Over The Chart DSL — Full Exploration (Archived)

## Status

**Archived 2026-07-09.** The complete exploration record (2026-07-08/09)
with every dated reversal preserved: the egui-as-runtime direction
explored and reversed, the layout model's evolution (DSL-owned →
egui-owned → avenger-layout), the widget-runtime decision history, and
the Slint deep-dive. Kept for archaeology — *why* the decisions went the
way they did.

The live, implementation-facing form lives in three distillations, which
supersede this document wherever they disagree:

- `../dashboard-layer.md` — the dashboard layer itself: component and
  binding model, DSL surface, engine representation, runtime, phasing.
- `../widgets.md` — the widget paradigm.
- `../dashboard-layout.md` — the layout model and its avenger-layout
  reuse.
