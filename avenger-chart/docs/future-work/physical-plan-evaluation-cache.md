...
| `PartitionSlotCache` | PER-EVALUATION memo of facet observed/domain slot values, string-keyed on the `Debug` format of the full logical plan plus filters and params. | AS BUILT 2026-07-11 (dissolution campaign): the session lifetime was dissolved — the memo now lives for one `evaluate` call (preview attempt and fallback share an instance). The stale-slot class (host `INSERT INTO` / re-registration invisible to the plan-Debug key) is fixed by re-execution through the content-versioned physical cache, pinned by enabled probes in `tests/core_integration/test_cache_dissolution.rs`. Measurement also showed the session memo was a net latency LOSS on width-change re-evaluation (70 vs 133 ms with it disabled); the "thin fingerprint-keyed memo" idea proved unnecessary. |
| `ScaleDomainCache` | PER-EVALUATION memo of `ScaleBuilder` domain artifacts keyed by compiled-object pointers, scope, and stringified params. | AS BUILT 2026-07-11 (dissolution campaign): the session lifetime was dissolved — pointer identity (`data_ptr`) cannot observe provider-interior mutation (host `INSERT INTO`) or table re-registration, and the reproduced stale-domain bug had no identity-source fix reachable from avenger. Within one evaluation the memo still serves repeated builder asks; cross-evaluation reuse of the domain QUERIES belongs to the physical cache (~33 ms warm-evaluation cost measured on a 6-cell wrap, previews unaffected — correctness won). Hot-reload survivability via structural keys remains future work. |
| `MaterializationCache` ready payloads | Stores queued/running/ready/error materialization entries and ready `MaterializationResult` payloads. Keys are already versioned structural hashes of the proto-serialized plan, transform, and params — computable without planning. | Keep scheduling, debounce/throttle, latest-ready identity, stale fallback, invalidation, AND payload storage — DECIDED 2026-07-10 (owner): ready payloads stay in MaterializationCache permanently, and pinning is NOT added to the physical cache. Stale fallback requires payloads to exist when policy reaches for them, which conflicts with the physical cache's core invariant (every entry evictable, reconstructible by re-execution, never load-bearing). Two layers, two contracts. |
...
3. Cost-aware admission:
   ...
   - hinted chart subtrees. (Pinned entries were considered here and
     REJECTED 2026-07-10: materialization payloads stay in
     MaterializationCache — see the disposition table.)
