...
| [categorical-raster-coloring.md](categorical-raster-coloring.md) | Research + API recommendation (2026-07-05): datashader-style categorical 3D rasters — `Rasterize2D::by(...)` plane dimension, mark overlay mode with `fill_by`/`opacity_by_total`, perceptual Oklab weighted mixing (with linear/srgb options), categorical fill domain inference and swatch legends via the ordinary ordinal-scale machinery. |
