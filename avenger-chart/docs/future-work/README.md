...
| [geo-raster-marks.md](geo-raster-marks.md) | Design ready for implementation: CRS-tagged rasters (`geometry.crs`, open string vocabulary, core stays CRS-ignorant) + `UniformRaster2D<Geo>` with tile-style identity/warped dispatch via a generalized `warped_raster_mesh`; enables datashader rasters over basemaps, Albers/blend warping, and EPSG:4326 climate grids. |
