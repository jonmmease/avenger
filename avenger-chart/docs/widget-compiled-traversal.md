...
| Materialization scheduling and prefetch planners | Required | Required when declared by a part | Implemented for item plans and declared part-local transforms. Scale-free `View::pixel_frame()` supplies logical-pixel frame parameters to the ordinary view/materialization pipeline without positional scales or guides; a bincode-round-tripped part emits and renders through an eager-scalar preview request. PixelFrame deliberately publishes no Cartesian guide prefetch planner, and asynchronous resource wake-up remains assigned to the first resource-bearing native/PixelFrame mark in W5. |
...
The W1 matrix is complete: every required branch is implemented or has an
explicit first-consumer gate. When a new compiled-content consumer is
