...
| Rendering and style/measurement caches | Required | Required | Implemented without a second item evaluation or CSS query. The durable prepared-item key contains only item-plan/validation params plus selection/store revisions: a referenced CSS custom-property repaint is a cache hit with zero item collects, while a query-param revision is one miss/collect. Complete theme/environment style/measurement-key gates remain pending. |
