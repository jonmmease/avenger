...
pub fn prepare_number_tick_format(
    ...
    let mut prepared = prepare_number_span_format(
        start,
        stop,
    ...
    if matches!(
        ...
        let tiers = if prepared.resolved.format_type == Some(FormatType::CompactShort) {
            &context.locale.extensions.compact_short
        } else {
            &context.locale.extensions.compact_long
        };
        if let Some(tier) = crate::format::select_compact_tier(start.abs().max(stop.abs()), tiers) {
            let fraction = match prepared.resolved.digit_spec {
                DigitSpec::Auto => decimal::exponent(tick_step(
                    start,
                    stop,
                    values.len().saturating_sub(1) as f64,
                ))
                .map_or(0, |step| (tier.exponent - step).max(0) as u8),
                DigitSpec::Precision(p) | DigitSpec::Fraction(p) | DigitSpec::Significant(p) => p,
            };
            // A common compact scale keeps the same unit on every tick.
            prepared.scale = 10_f64.powi(-tier.exponent);
            prepared.resolved.format_type = Some(FormatType::Fixed);
            prepared.resolved.digit_spec = DigitSpec::Fraction(fraction);
            let (prefix, suffix) = tier
                .other
                .split_once("{0}")
                .expect("validated compact pattern");
            prepared.prefix = prefix.into();
            prepared.suffix = suffix.into();
        }
    }
    Ok(prepared)
}
