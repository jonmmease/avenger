...
// Binary64 values are exact ratios. Decimal rounding chooses the larger integer at a tie,
// as required by ECMAScript toFixed, toPrecision, and toExponential.
...
pub(crate) fn parts(value: f64, precision: usize) -> (String, i32) {
    ...
    if digits.len() < precision {
        exponent -= 1;
        digits = rounded_scaled(value, precision as i32 - exponent - 1).to_str_radix(10);
    }
    if digits.len() > precision {
        digits.pop();
        exponent += 1;
