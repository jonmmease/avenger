...
`avenger-format-number` implements valid D3 number formats with D3 locale definitions. The crate has no chart or Typst dependency.
...
`prepare_number_span_format` implements Vega's domain/count-based precision selection. `prepare_number_float_format` implements Vega's automatic floating-point labels. `prepare_number_prefix_format` fixes a D3 SI unit using a reference value. Existing native tick sets can use `prepare_number_tick_format`.
