//! Core legend renderer contracts.
...
#[derive(Clone)]
pub enum LegendRendererSelection {
    BuiltIn(LegendRendererKind),
    Custom(Arc<dyn LegendRenderer>),
}

impl LegendRendererSelection {
    pub fn theme_selector(&self) -> Option<&'static str> {
        match self {
            Self::BuiltIn(kind) => Some(kind.theme_selector()),
            Self::Custom(_) => None,
        }
    }
}
