...
use crate::{
    ...
    EvaluationContext, EventDatumFieldSpec, GuideEventDatumRows, GuideOverflowPhase, LayoutBounds,
    MeasurementResult, OverflowSpaceRequirement, SharingLevel, Theme,
    guide_sharing::{
        AxisOwnershipMode, AxisVisibility, ChildFrameGuideSharingView, FacetGuideSharingView,
    },
};

/// Stable render-time context available to coordinate guides.
#[derive(Clone, Copy)]
pub struct GuideRenderContext<'a> {
    eval: Option<&'a EvaluationContext>,
    plot_width: f32,
    plot_height: f32,
}

impl<'a> GuideRenderContext<'a> {
    #[doc(hidden)]
    ...
    #[doc(hidden)]
    pub fn without_resource_sink(plot_width: f32, plot_height: f32) -> Self {
        Self {
            eval: None,
            plot_width,
            plot_height,
    ...
    pub fn eval(&self) -> Option<&'a EvaluationContext> {
        self.eval
    }
    ...
    /// Record a resource required by guide-rendered scene marks.
