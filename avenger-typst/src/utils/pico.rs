    ...
    impl EncodingError {
        pub const fn message(&self) -> &'static str {
            match self {
                ...
                Self::BadChar => "can only auto-intern the chars 'a'-'z', '1'-'4', and '-'",
            }
        }
    }
}
...
#[track_caller]
const fn failed_to_compile_time_intern(error: bitcode::EncodingError, string: &'static str) -> ! {
    ...
    let Ok(message) = std::str::from_utf8(slice) else {
        panic!()
    };
