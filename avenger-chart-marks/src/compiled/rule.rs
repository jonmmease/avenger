...
use crate::{compiled::util, rule_channel_defaults};
use avenger_chart_core::{
    ...
    MarkAdjustmentSpec, MarkEvaluationFrame, MarkRuntimeContext, PrimitiveMarkEffects,
...
impl CompiledMarkCore for CompiledRule {
...
#[typetag::serde(name = "CompiledCartesianRule")]
#[cfg_attr(target_arch = "wasm32", async_trait::async_trait(?Send))]
#[cfg_attr(not(target_arch = "wasm32"), async_trait::async_trait)]
impl CompiledMark for CompiledRule {
    async fn render_from_data(
        ...
        let start = util::transform_point_channels(self, data, scalars, context, coord, "x", "y")?;
        let end = util::transform_point_channels(self, data, scalars, context, coord, "x2", "y2")?;
                ...
                match assignment.channel.as_str() {
                    ...
                    channel => {
                        return Err(AvengerChartError::InvalidArgument(format!(
                            "Rule adjustment channel '{channel}' is not implemented yet"
                        )));
                    }
                }
            }
