...
use crate::{
    compiled::{CompiledText, util},
    symbol_channel_defaults, symbol_legend_renderer_kind,
};
use avenger_chart_core::{
    ...
    DerivedRuleMarkSpec, DerivedSymbolMarkSpec, DerivedTextMarkSpec, LegendRendererSelection,
    MarkAdjustmentSpec, MarkEvaluationFrame, MarkRenderContext, MarkRuntimeContext,
    PhysicalScalarExpressionSpec, PhysicalScalarProgramOptions, PlotAreaInfo, PointGeometry,
...
impl CompiledSymbol {
    pub fn new(state: CompiledMarkState, effects: PrimitiveMarkEffects) -> Self {
        Self { state, effects }
    }
}

impl CompiledMarkCore for CompiledSymbol {
...
#[typetag::serde(name = "CompiledCartesianSymbol")]
#[cfg_attr(target_arch = "wasm32", async_trait::async_trait(?Send))]
#[cfg_attr(not(target_arch = "wasm32"), async_trait::async_trait)]
impl CompiledMark for CompiledSymbol {
        ...
        if include_item_frame {
            let source_frame = source_frame.ok_or_else(|| {
                AvengerChartError::InternalError(
                    "Derived Symbol rendering expected a source item frame".to_string(),
                )
            })?;
                ...
                match derived {
                    ...
                    other => {
                        return Err(AvengerChartError::InvalidArgument(format!(
                            "Symbol cannot derive {other:?} yet"
                        )));
                    }
                }
            }
...
impl CompiledSymbol {
    ...
    fn render_derived_text(
        ...
        let len = source_frame.len();
        let text_defaults = CompiledText {
        ...
        for adjustment in &self.effects.adjustments {
            ...
            for (index, channel) in output_channels.iter().enumerate() {
                ...
                match channel.as_str() {
                    ...
                    _ => unreachable!("validated symbol adjustment channel"),
                }
            }
        }
    ...
    if let Some(data) = data {
        if data.num_rows() != len {
            return Err(AvengerChartError::InternalError(format!(
                "Symbol adjustment data row count {} did not match item count {len}",
                data.num_rows()
            )));
        }
