...
pub fn transform_point_channels<M>(
    mark: &M,
    ...
    geometry
        ...
        .ok_or_else(|| {
            AvengerChartError::CoordinateSystemError(
                "Failed to downcast transformed point to PointGeometry".to_string(),
            )
        })
}

pub fn scene_len(data: Option<&RecordBatch>) -> u32 {
    data.map_or(1, |data| data.num_rows()) as u32
}

pub fn adjustment_transform_context(
    context: &dyn MarkRuntimeContext,
