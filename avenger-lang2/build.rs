...
pub enum {node_name}<'tree> {{"
            ...
            writeln!(
                out_file,
                r#"impl<'tree> crate::ts_typed_ast::AstNode<'tree> for {node_name}<'tree> {{
...
"#
        ...
        crate::ts_typed_ast::Children::new(crate::ts_typed_ast::AstNode::node(self), std::num::NonZeroU16::new({field_id}).unwrap())
    }}"#
                        )
                        .unwrap();
                    } else if required {
                        writeln!(
                            out_file,
                            r#"    pub fn {method}(&self) -> Result<{type_}<'tree>, crate::ts_typed_ast::MissingNodeChildError<'tree>> {{
        let node = crate::ts_typed_ast::AstNode::node(self);
        node
            .child_by_field_id({field_id})
            .and_then(crate::ts_typed_ast::AstNode::cast)
            .ok_or_else(|| crate::ts_typed_ast::MissingNodeChildError::new(node, {field_id}))
    }}"#
                        )
                        .unwrap();
                    } else {
        ...
        crate::ts_typed_ast::AstNode::node(self)
            .child_by_field_id({field_id})
            .and_then(crate::ts_typed_ast::AstNode::cast)
    }}"#
                        )
