...
pub trait ScaleImpl: Debug + Send + Sync + 'static {
    ...
    fn legend_entries(
        ...
        Ok(None) // Let the caller extract entries from the domain.
    }
...
impl ConfiguredScale {
    ...
    pub fn format_dates(
        ...
        Ok(ScalarOrArray::new_array(
            crate::formatter::DateFormatter::format(
                self.config.context.formatters.civil_datetime()?,
                values,
                None,
            )?,
        ))
    ...
    pub fn format_timestamps(
        ...
        Ok(ScalarOrArray::new_array(
            crate::formatter::TimestampFormatter::format(
                self.config.context.formatters.civil_datetime()?,
                values,
                None,
            )?,
    ...
    pub fn format_timestamptz(
        &self,
        values: &[Option<DateTime<Utc>>],
    ) -> Result<ScalarOrArray<String>, AvengerScaleError> {
        Ok(ScalarOrArray::new_array(
            crate::formatter::TimestamptzFormatter::format(
                self.config.context.formatters.instant()?,
                values,
                None,
            )?,
        ))
