//! Fit a linear domain to data points with lower and upper pixel extents.
//!
//! For each value `d`, the mapped position must be at least its lower radius
//! from the start of the range and its upper radius from the end. Distinct
//! values use the narrowest feasible domain found by active-constraint search.
//! Coincident values have no unique minimum width, so they use a conventional
//! positive span based on their largest extents. Empty data returns `(0, 1)`.
//!
//! Inputs must be finite, radii nonnegative, and the range positive. Invalid
//! input, infeasible marker extents, and unrepresentable solutions return errors.
//! Callers can choose a visual fallback explicitly after handling those errors.
//!
//! Support filtering sorts the points and retains candidates with increasing
//! extents. Each candidate pair is checked against every input point, for a cost
//! of O(n log n + k_left * k_right * n).
//!
//! ```
...
//!     &[0.0, 10.0], &[5.0, 5.0], &[5.0, 5.0], 100.0,
//! ).unwrap();
//! assert!(domain.0 < 0.0 && domain.1 > 10.0);
//! ```
...
///
/// # Returns
/// * `Ok((d_min, d_max))` - A fitted domain. Coincident points use a conventional
///   positive span because there is no unique minimum width.
/// * `Err(AvengerScaleError)` - Invalid input, infeasibility, or no representable solution
pub fn compute_domain_from_data_with_padding_linear(
    ...
    if !range.is_finite() || range <= 0.0 {
        return Err(DomainError::InvalidInput(
            "Screen width must be finite and positive".to_string(),
        )
        .into());
    }

    if domain_points.iter().any(|point| !point.is_finite())
        || radius_lower
            .iter()
            .chain(radius_upper)
    ...
    {
        return Err(DomainError::InvalidInput(
            "Points must be finite and radii must be finite and nonnegative".to_string(),
        )
        .into());
    }
    if radius_lower
        .iter()
        .zip(radius_upper)
        .any(|(lower, upper)| lower + upper > range)
    {
        return Err(DomainError::Infeasible(
            "A marker's combined radii exceed the screen width".to_string(),
        )
        .into());
    }
    ...
    let data_min = domain_points.iter().copied().fold(f64::INFINITY, f64::min);
    let data_max = domain_points
        .iter()
        .copied()
        .fold(f64::NEG_INFINITY, f64::max);
    if data_min == data_max {
        let lower = radius_lower.iter().copied().fold(0.0, f64::max);
        let upper = radius_upper.iter().copied().fold(0.0, f64::max);
        let solution = solve_single_point(data_min, lower, upper, range)?;
        return if is_feasible(domain_points, radius_lower, radius_upper, range, solution) {
            Ok(solution)
        } else {
            Err(DomainError::NoSolution.into())
        };
    }
    if !(data_max - data_min).is_finite() {
        return Err(DomainError::InvalidInput(
            "Data extent exceeds the finite numeric range".to_string(),
        )
        .into());
    }
    ...
    best_solution.ok_or_else(|| DomainError::NoSolution.into())
}
...
fn solve_single_point(
    ...
    // Coincident data has no unique minimum domain width. Retain a conventional
    // span proportional to the extents, with the correct asymmetric placement.
    let d_min = d - radius_lower;
    let d_max = d + radius_upper;
...
fn is_feasible(
    ...
    if !solution.0.is_finite()
        || !solution.1.is_finite()
        || !(solution.1 - solution.0).is_finite()
...
mod tests {
    ...
    #[test]
    fn oversized_markers_return_infeasible_for_any_point_count() {
        for points in [vec![0.0], vec![0.0, 1.0], vec![0.0, 0.0]] {
            let radii = vec![60.0; points.len()];
            assert!(matches!(
                compute_domain_from_data_with_padding_linear(&points, &radii, &radii, 100.0),
                Err(AvengerScaleError::DomainFromPaddingError(
                    DomainError::Infeasible(_)
                ))
            ));
    ...
    fn invalid_numeric_inputs_return_errors() {
        for invalid in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
            assert!(compute_domain_from_data_with_padding_linear(
                &[0.0, invalid],
                &[0.0; 2],
                &[0.0; 2],
                100.0
        ...
        assert!(
            compute_domain_from_data_with_padding_linear(&[0.0], &[-1.0], &[0.0], 100.0).is_err()
        );
    }

    #[test]
    fn coincident_points_use_the_largest_extents() {
        let result = compute_domain_from_data_with_padding_linear(
            &[2.0, 2.0],
            &[30.0, 10.0],
            &[5.0, 40.0],
    ...
    #[test]
    fn fitted_domains_match_an_independent_affine_feasibility_search() {
        // Search the maximum slope a for x = a * value + b. A feasible b
        // lies in the intersection of all marker placement intervals.
        let mut seed = 17_u64;
        let mut random = || {
        ...
        for _ in 0..200 {
            let points: Vec<_> = (0..6).map(|_| random() * 20.0).collect();
            let lower: Vec<_> = (0..6).map(|_| random() * 40.0).collect();
            let upper: Vec<_> = (0..6).map(|_| random() * 40.0).collect();
            let feasible = |slope: f64| {
                let low = points
                    .iter()
                    .zip(&lower)
                    .map(|(d, r)| r - slope * d)
            ...
            let result =
                compute_domain_from_data_with_padding_linear(&points, &lower, &upper, 100.0)
                    .unwrap();
            let width = result.1 - result.0;
            assert!((width - 100.0 / low).abs() < 1e-8);
            for ((point, lower), upper) in points.iter().zip(&lower).zip(&upper) {
                let pixel = (point - result.0) / width * 100.0;
                assert!(pixel >= lower - 1e-8 && pixel + upper <= 100.0 + 1e-8);
            }
        }
    }
}
