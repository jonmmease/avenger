        ...
        for i in 0..=thresholds.len() {
            let (label, repr_value) = if i == 0 {
                ...
                if let Some(&first) = thresholds.first() {
                    let formatted = formatter.format(first as f64);
                    (format!("< {}", formatted.text), first - 1.0)
                } else {
                    continue;
                }
                ...
                if let Some(&last) = thresholds.last() {
                    let formatted = formatter.format(last as f64);
                    (format!("≥ {}", formatted.text), last + 1.0)
                } else {
                    continue;
                }
            } else {
                ...
                let formatted_prev = formatter.format(prev as f64);
                let formatted_next = formatter.format(next as f64);
                (
                    format!("{} - {}", formatted_prev.text, formatted_next.text),
                    (prev + next) / 2.0,
                )
            };
