...
impl ArrowScale for LinearScale {
    ...
    fn scale_to_numeric(
        ...
        let (domain_start, domain_end) = apply_nice(
...
fn apply_nice(
    domain: (f32, f32),
    nice_option: Option<&ScalarValue>,
) -> Result<(f32, f32), AvengerScaleError> {
    // Extract count, or return raw domain if no nice option
    let count = if let Some(nice_option) = nice_option {
        if nice_option.data_type().is_numeric() {
            nice_option.as_f32()?
        } else if nice_option == &ScalarValue::from(true) {
            10.0
        } else {
            return Ok(domain);
        }
...
mod tests {
    ...
    fn test_scale() -> Result<(), AvengerScaleError> {
        ...
        let config = ScaleConfig {
            domain: vec![10.0.into(), 30.0.into()],
            range: vec![0.0.into(), 100.0.into()],
    ...
    fn test_scale_degenerate() -> Result<(), AvengerScaleError> {
        ...
        let config = ScaleConfig {
            domain: vec![10.0.into(), 10.0.into()],
            range: vec![0.0.into(), 100.0.into()],
            options: vec![("clamp".to_string(), true.into())]
                .into_iter()
                .collect(),
    ...
    fn test_degenerate_cases() -> Result<(), AvengerScaleError> {
        ...
        let config = ScaleConfig {
            domain: vec![1.0.into(), 1.0.into()],
            range: vec![0.0.into(), 0.0.into()],
        ...
        let config = ScaleConfig {
            domain: vec![0.0.into(), 10.0.into()],
            range: vec![1.0.into(), 1.0.into()],
            options: vec![("clamp".to_string(), false.into())]
                .into_iter()
                .collect(),
        };
    ...
    fn test_ticks_span_zero() -> Result<(), AvengerScaleError> {
        let config = ScaleConfig {
            domain: vec![(-100.0).into(), 100.0.into()],
            range: vec![0.0.into(), 100.0.into()],
            options: vec![].into_iter().collect(),
        };
    ...
    fn test_nice_convergence() -> Result<(), AvengerScaleError> {
        let config = ScaleConfig {
            domain: vec![1.1.into(), 10.9.into()],
            range: vec![0.0.into(), 100.0.into()],
            options: vec![].into_iter().collect(),
        };

        let niced_domain = apply_nice(config.numeric_interval_domain()?, Some(&10.0.into()))?;
