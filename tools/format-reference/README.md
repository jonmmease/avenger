...
Datetime definitions come from d3-time-format 4.1.0. The generator reads bundled `en-US` and additional definitions from `avenger-format-datetime-d3/tests/fixtures/locales/`, with licenses beside both sets. The full directive matrix runs in UTC. Separate processes exercise automatic selection at local calendar boundaries and targeted timezone transitions. Rust tests cover strict pattern validation and the intentional `%j` calendar-ordinal exception.
