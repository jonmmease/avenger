        ...
        if include_item_frame {
            let source_frame = source_frame.ok_or_else(|| {
                AvengerChartError::InternalError(
                    "Derived WebMercator symbol rendering expected a source item frame".to_string(),
                )
            })?;
                ...
                match derived {
                    ...
                    other => {
                        return Err(AvengerChartError::InvalidArgument(format!(
                            "Symbol<WebMercator> cannot derive {other:?} yet"
                        )));
                    }
                }
            }
...
impl CompiledWebMercatorSymbol {
    ...
    fn render_derived_text(
        ...
        let len = source_frame.len();
        let text_defaults = DerivedWebMercatorTextDefaults {
        ...
        for adjustment in &self.effects.adjustments {
            ...
            for (index, channel) in output_channels.iter().enumerate() {
                ...
                match channel.as_str() {
                    ...
                    _ => unreachable!("validated WebMercator symbol adjustment channel"),
                }
            }
        }
...
#[derive(Clone, Serialize, Deserialize)]
struct DerivedWebMercatorTextDefaults {
    state: CompiledMarkState,
    #[serde(default)]
    effects: PrimitiveMarkEffects,
}

impl CompiledMarkCore for DerivedWebMercatorTextDefaults {
    fn state(&self) -> &CompiledMarkState {
        &self.state
    }
    ...
    if let Some(data) = data {
        if data.num_rows() != len {
            return Err(AvengerChartError::InternalError(format!(
                "WebMercator symbol adjustment data row count {} did not match item count {len}",
                data.num_rows()
            )));
        }
