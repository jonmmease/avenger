...
async fn independent_producers_preserve_fixed_predicates_and_correlated_tuples() -> TestResult {
    let focus = producer("brush", view("focus"), &["delay", "region"]);
...
async fn facet_addresses_exclude_only_the_exact_view_instance() -> TestResult {
    let east = ViewId::new("east-histogram")?;
    let west = ViewId::new("west-histogram")?;
    let focus = producer("brush", east.clone(), &["delay"]);
...
async fn mapped_pixel_dimensions_cover_inactive_brushes_and_reject_changed_definitions(
    ...
    let filter = ConsumerFilter::new(view("target"), SelectionFilter::cross_filter([&id()]))
        .with_projection(&original, &ProjectionId::new("x")?, col("delay"))?;
...
async fn unsupported_composition_retains_full_membership_and_invalid_names_are_errors() -> TestResult
{
    ...
    let foreign = producer("foreign", view("foreign"), &["id"]);
    assert!(membership()
...
#[tokio::test]
async fn row_id_projections_support_predicates_and_retained_dimensions() -> TestResult {
    let focus = producer("ids", view("source"), &["id"]);
    let state = state(Resolution::Intersect).set(&focus, values("id", [1_i64.into()]))?;
    let filter = cross(view("target"));
    let predicates = filter.predicates(&state, &focus)?;
    assert_eq!(predicates.split().unwrap().dimensions(), &[col("id")]);
    assert_eq!(
        selected(flights(), predicates.full().clone()).await,
        vec![1]
    );
