...
use sqlparser::{
    ast::{
        AccessExpr, Expr as SqlExpr, Function, FunctionArg, FunctionArgExpr, FunctionArgOperator,
        FunctionArguments, Ident, Query as SqlQuery, Visitor, VisitorMut,
    },
    tokenizer::Span,
};
...
impl FunctionBindingExpander {
    ...
    fn push_component_instance(&mut self, statement: &ComponentProp) {
        self.component_instances
            .insert(statement.name().to_string(), statement.statements.clone());
    }
...
impl VisitorMut for FunctionBindingExpander {
    ...
    fn pre_visit_expr(&mut self, expr: &mut SqlExpr) -> ControlFlow<Self::Break> {
        if let SqlExpr::CompoundFieldAccess { root, access_chain } = expr.clone() {
            if let SqlExpr::Function(Function { name, args, .. }) = root.as_ref() {
                if name.0[0].value.starts_with("@") {
                    ...
                    for access in access_chain.iter() {
                        if let AccessExpr::Dot(expr) = access {
                            ...
                            return ControlFlow::Break(Err(AvengerLangError::InternalError(
                                format!("Expected a dot access expression, got {:?}", access),
                            )));
                        }
                    }
                            ...
                            } = arg
                            {
                                ...
                                if operator != &FunctionArgOperator::Assignment {
                                    return ControlFlow::Break(Err(
                                        AvengerLangError::InternalError(format!(
                                ...
                                if let FunctionArgExpr::Expr(expr) = arg {
                                    named_args.push((name.value.clone(), expr.clone()));
                                } else {
                                    return ControlFlow::Break(Err(
                                        AvengerLangError::InternalError(format!(
                                            "Expected an expression for function argument, got {:?}",
                                            arg
                                        )),
                                    ));
                                }
                                ...
                                return ControlFlow::Break(Err(AvengerLangError::InternalError(
                                    format!("Expected a named function argument, got {:?}", arg),
                                )));
                            }
                        }
                    } else {
                    ...
                    let bound_instance_name =
                        create_bound_instance_name(&instance_name, &named_args);
                    ...
                    let mut parts = vec![Ident::new(format!("@{}", bound_instance_name))];
                    for access in access_chain.iter() {
                        if let AccessExpr::Dot(expr) = access {
                            parts.push(Ident::new(expr.to_string()));
                        } else {
                            return ControlFlow::Break(Err(AvengerLangError::InternalError(
                                format!("Expected a dot access expression, got {:?}", access),
                            )));
        ...
        for (bound_instance_name, instance) in self.bound_instances.iter() {
            if instance.path == comp_path {
                let Some(inner_statements) = self.component_instances.get(&instance.instance_name)
                    ...
                    return ControlFlow::Break(Err(AvengerLangError::InternalError(format!(
                        "Expected component instance to exist for {}",
                        bound_instance_name
                    ))));
                };
                ...
                let Ok(inner_statements) = override_statements_with_params(
                    ...
                    return ControlFlow::Break(Err(AvengerLangError::InternalError(format!(
                        "Error overriding statements for {}",
                        bound_instance_name
                    ))));
                };
                ...
                statement
                    .statements
                    .push(Statement::ComponentProp(ComponentProp {
                        prop_name: Some(Ident::new(bound_instance_name.clone())),
                        qualifier: None,
                        component_keyword: Some(KeywordComp {
                            span: Span::empty(),
                        }),
...
fn override_statements_with_params(
    statements: &[Statement],
    _instance_name: &str,
    params: &[(String, SqlExpr)],
) -> Result<Vec<Statement>, AvengerLangError> {
            ...
            Statement::DatasetProp(dataset_prop) => {
                ...
                if let Some(expr) = params.get(dataset_prop.name()) {
                    return Err(AvengerLangError::InternalError(format!(
                        "Function with dataset prop not supported: {}",
                        dataset_prop.name()
                    )));
                } else {
                    // // Map some props to original component instance?
...
pub fn build_select_star_from_name(
    instance_name: &str,
    prop_name: &str,
) -> Result<Box<SqlQuery>, AvengerLangError> {
...
mod tests {
    ...
    fn test_create_bound_instance_name() {
        ...
        "#;
        let mut parser = AvengerParser::new(src, "Test", ".").unwrap();
