...
use sg2d::marks::value::{ColorOrGradient, Gradient};
...
impl GradientRectInstance {
    pub fn from_spec(mark: &RectMark) -> (Vec<GradientRectInstance>, image::RgbaImage) {
        ...
        // Write gradients
        assert!(
            mark.gradients.len() < (GRADIENT_TEXTURE_HEIGHT / 2) as usize,
            "Exceeded max number of unique gradients"
        );

        for (pos, grad) in mark.gradients.iter().enumerate() {
        ...
        let compute_color = |color_or_gradient: &ColorOrGradient| -> [f32; 4] {
            match color_or_gradient {
                ...
                ColorOrGradient::GradientIndex(grad_idx) => {
                    ...
                    let grad_coord =
                        (*grad_idx as f32 / 128.0) + 1.0 / (GRADIENT_TEXTURE_HEIGHT as f32 * 2.0);
